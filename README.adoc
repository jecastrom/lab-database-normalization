...
In the "film" table, there are several ambiguous columns. Furthermore, the number of available languages is determined more by the copy version (i.e., inventory) than by film per se. Delete column "language_id" from the "film" table. Create columns "dubbing_language" and "language_subtitles" in the "inventory" table: FK for language id.
